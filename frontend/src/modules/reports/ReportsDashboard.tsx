...
function openChatHrefFromReport(row: ReportSummary): string {
  ...
  if (row.main_agent_id) {
    qs.set("mainAgent", row.main_agent_id);
  }
  return `/?${qs.toString()}`;
}
...
/** `/reports` route: saved SQL report PDFs + navigation. */
        ...
        <div className="min-h-0 flex-1 overflow-auto p-8">
          {loading ? (
            ...
            <p className="text-sm text-zinc-500">
              No saved reports yet. Open a chat, run SQL, then use <strong>Save report</strong> on the analysis
              card.
            </p>
          ) : (
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-xs text-zinc-500">
                      ...
                      <code className="rounded bg-zinc-100 px-1 text-[11px]">{row.thread_id.slice(0, 8)}…</code> ·{" "}
